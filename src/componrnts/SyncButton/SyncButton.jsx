...
export const useBackendSync = (onSyncComplete) => {
    ...
    const label = syncing ? `Yuklanmoqda${dots}` : "Yuklash";
    ...
    const loader = syncing ? (
        <Loader message={syncMessage} current={progress.current} total={progress.total} />
    ) : null;
    ...
    return {
        ...
        icon,
        loader,
        onClick: !syncing ? handleSync : handleCancel,
    };
};

const SyncButton = ({ onSyncComplete }) => {
    const { label, icon, loader, onClick } = useBackendSync(onSyncComplete);
